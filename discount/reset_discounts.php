...
try {
	...
	$ch = curl_init();		
	$url = 'http://10.8.0.1/Dropbox/discount/get_discounts_pusat.php?id='.$BRANCH_ID;
	...
	if(count($rows)>0) {
		...
		foreach($rows as $row) {
			...
			$sql = "select count(1) as cnt from discounts where code='".$row['code']."'";			
			$row_cnt = $conn->fetch_row($sql);
			if($row_cnt!=null && $row_cnt['cnt']>0) {				
			}
			else {
				
				$sql = "insert into discounts (code, enabled, start_date, end_date)
				 values (
				'".$row['code']."',
				'".$row['enabled']."',
				'".$row['start_date']."',
